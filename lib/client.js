...
// TODO - Verify merge order between global, global defaults and method parameters.
// TODO - Add start e stop methods to the client.
// TODO - Remove close from prototype.
// TODO - Move some helper request functions into request class.
...
function flush(self) {
    ...
    if ( (self.aggregatedBuffer.bufferSize + self.nonAggregatedBuffer.bufferSize) > 0) {
        if (self.dryRun) {
            if (self.logger) {
                if (self.nonAggregatedBuffer.bufferSize > 0) {
                    self.logger.debug('Flushing metrics (non aggregated): ' + self.nonAggregatedBuffer);
                }
                if (self.aggregatedBuffer.bufferSize > 0) {
                    self.logger.debug('Flushing metrics (aggregated): ' + self.aggregatedBuffer);
                }
            }
                ...
                case 'http':
                    ...
                    if (self.aggregatedBuffer.bufferSize > 0) {
                        for (var agg in self.aggregatedBuffer) {
                            for (var aggFreq in self.aggregatedBuffe[agg]) {
                                if (self.aggregatedBuffer[agg][aggFreq].length > 0) {
        ...
        if (self.systemStats) {
            if (self.aggregatedBuffer.bufferSize > 0 && self.transport === "http") {
                self.put('buffer_aggregated.flush_length', self.aggregatedBuffer.bufferSize, {agg: ['avg']});
            }
            if (self.nonAggregatedBuffer.bufferSize > 0) {
                self.put('buffer.flush_length', self.nonAggregatedBuffer.bufferSize, {agg: ['avg']});
            }
        }
