...
function flush(self) {
    ...
    if ( (self.aggregatedBuffer.bufferSize + self.nonAggregatedBuffer.bufferSize) > 0) {
        if (self.dryRun) {
            if (self.logger) {
                self.logger.debug('Flushing metrics: ' + self.buffer);
            }
        } else {
...
Client.prototype.aggregatedTimer = function (name, timestamp, agg, aggFreq, parameters) {
    putAggregatedMetric(this, this.default.timer, 'timer.' + name, timestamp, agg, aggFreq, parameters);
};
...
Client.prototype.timer = function (name, timestamp, parameters) {
    putNonAggregatedMetric(this, this.default.timer, 'timer.' + name, timestamp, parameters);
};
